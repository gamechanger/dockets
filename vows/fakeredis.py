"""
The below is copied almost entirely from
https://github.com/jamesls/fakeredis. Some changes were made to
support additional operations and to isolate FakeRedis instances.
"""
...
class FakePipeline(object):
    ...
    def execute(self):
        ...
        if self.watching:
            mismatches = [
                (k, v, u) for (k, v, u) in
                [(k, v, self.owner._db.get(k)) for (k, v) in self.watching.items()]
                if v != u]
            if mismatches:
                self.commands = []
